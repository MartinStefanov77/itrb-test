...
/* ---- Careers listing: filters + search (runs after i18n) ------------- */
(function initCareersListing() {
  ...
  /** Native select stays for i18n + values; custom button + listbox for styling */
  ...
  /** URL segment from English card title — stable when user switches BG/EN. */
  function slugifyCareerTitle(raw) {
  ...
  function setCareersJobDetailLinks() {
    allJobs().forEach((card) => {
      const titleEl = jobTitleEl(card);
      const key = titleEl?.getAttribute("data-i18n");
      const link = card.querySelector(".careers-card-aside a[href]");
      if (!key || !link || !key.endsWith(".cardTitle")) return;
      const rawHref = link.getAttribute("href") || "";
      if (/^https?:\/\//i.test(rawHref)) return;
      const titleEn = translations.en[key];
      if (typeof titleEn !== "string") return;
      link.setAttribute("href", `careers-job-${slugifyCareerTitle(titleEn)}.html`);
    });
  }

  function haystackMatchesQuery(titleText, extraAttr, q) {
    const needle = q.trim().toLowerCase();
    if (needle.length < 3) return true;
    const haystack = `${(titleText || "").toLowerCase()} ${(extraAttr || "").toLowerCase()}`;
  ...
  function jobMatchesSearch(card, q) {
    ...
    return haystackMatchesQuery(title, extra, q);
  }
  ...
  function buildSuggestions() {
    ...
    const seen = new Set();
    allJobs().forEach((card) => {
      const title = (jobTitleEl(card)?.textContent || "").trim();
      if (!title || seen.has(title)) return;
      const extra = card.getAttribute("data-search") || "";
      if (!haystackMatchesQuery(title, extra, searchInput.value)) return;
      seen.add(title);
