...
// ── Configuration ─────────────────────────────────────────────────────────────
const RECIPIENT_EMAIL = 'polina.lozanova@itrb.org';
...
export async function POST(req: NextRequest) {
  ...
  const name    = get('name').trim().replace(/<[^>]*>/g, '');
  const email   = get('email').trim();
  const subject = get('subject').trim().replace(/<[^>]*>/g, '');
  const message = get('message').trim().replace(/<[^>]*>/g, '');
  ...
  try {
    ...
    return NextResponse.json(
      { success: true, message: 'Thank you! Your message has been sent. We will get back to you shortly.' },
      { status: 200 }
    );
  } catch (err) {
    console.error('[contact] mail error:', err);
    return NextResponse.json(
      { success: false, message: `Failed to send the message. Please try again or email us directly at ${RECIPIENT_EMAIL}.` },
      { status: 500 }
    );
  }
}
